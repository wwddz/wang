...
//                根据滚动的距离分别修改透明度
    ...
    $('.user_name_a').mouseenter(function () {
        if (!$('.personal_nav').is(":visible")) {
            $('.personal_nav').show().animate({height: "auto"}, 500);
        }
    });
...
jQuery(document).ready(function ($) {
    ...
    //                 window.history.pushState('forward', null,'./?yzqandXYF=后退不了了吧，我故意设置的');
    //             }
    //         }
    //     });
    //     window.history.pushState('forward', '/?yzq','./?yzqandXYF='+new Date().getTime());
    // }
    //监听关闭事件
